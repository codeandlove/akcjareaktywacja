...
const Account = props => {
    const {openSidebar, closeSidebar, profile, auth, auth: {uid}, uploader, firebase} = props;
    ...
    const panes = [
        { menuItem: 'Witaj', render: accountWelcome },
        { menuItem: 'Twoje konto', render: accountSettings },
        { menuItem: 'Wyloguj się', render: accountLogout }
    ];
    ...
    const saveProfile = () => {
        ...
        verifyCaptcha(props, 'updateAccount').then(token => {
            if(token) {
                const usersRef = firebase.database().ref('/users');

                //Check if nick is unique
                usersRef.orderByChild('displayNick').equalTo(nick).once('value').then(snapshot => {
                    const isItMe = uid === (snapshot.val() && Object.keys(snapshot.val())[0]);
                    ...
                    if(!snapshot.val() || isItMe) {
                        ...
                        firebase.update(`users/${uid}`, data, () => {
...
export default enhance(withGoogleReCaptcha(Account));
